...
def test_unseen_symbol_gets_uniform_prior():
    from genmeta.predicates import object_meanings

    meanings = object_meanings()
    agent = Agent(id=0, lexicon={})
    agent.ensure_symbol("new_symbol", meanings)
    assert set(agent.lexicon["new_symbol"]) == {m.id for m in meanings}
    assert abs(sum(agent.lexicon["new_symbol"].values()) - 1.0) < 1e-9
